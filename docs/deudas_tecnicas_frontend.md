...
## Etapa 2 — Primitivos visuales sueltos · ✅ COMPLETA
...
de la Etapa 3 (que vuelve a `TabBarberos`). Ambas deudas (#17, #41) resueltas — ver apéndice.
...
### #41 — Wrapper "label + `InputTiempo`" duplicado — candidato a primitivo (`CampoTiempo`) · Baja · ✅ (2026-06-03)
...
**Resuelta (2026-06-03, branch `fix/deudas-frontend`) — opción "prop `label` en `InputTiempo`" (no primitivo nuevo).** Se descartó promover un `CampoTiempo` aparte por ser un primitivo casi-igual a `Field` (mismo criterio que #23/CampoFijo: plegar el modo en el primitivo existente antes que proliferar). Es **§7.4** (variante sobre un primitivo que ya existe), no §7.1, y *reduce* código en vez de sumarlo. Se actuó al **2º uso** (no se esperó al 3º) siguiendo la práctica del proyecto (#14 `LogoCirculo` se promovió con 2 usos citando §7.1) y porque la duplicación es concreta, no hipotética: el chrome del eyebrow estaba copiado **3 veces** (`Field` + `CampoTiempo` + el inline de `BloqueFeriados`).
**Cambio:** `InputTiempo` suma prop **`label` opcional** → con `label` se auto-envuelve en `<label>` + eyebrow (espejo de `Field`, asociación implícita, sin `aria-label`); sin `label` queda el input pelado idéntico a hoy (los **4 usos pelados** —editor de horario en `TabBarberos` y horario semanal en `BloqueHorarioAtencion`— no se tocan, siguen con `ariaLabel`). `TabBarberos`: se **borró** el sub-componente `CampoTiempo`; sus 2 callsites (Desde/Hasta) pasan a `<InputTiempo label=… full>` envueltos en `<div flex:1 minWidth:180>` (el layout lado-a-lado es responsabilidad del padre). `BloqueFeriados`: el wrapper inline "Fecha" → `<InputTiempo label="Fecha" type="date" full>`. Neto: eyebrow copiado 3 → 2 veces (`Field` + `InputTiempo`), sin primitivo nuevo. **Cambio consciente menor:** en `BloqueFeriados` el nombre accesible del input pasó de `"Fecha del feriado"` (viejo `ariaLabel`) a `"Fecha"` (la label visible, ahora fuente del nombre accesible, como en `Field`) — coherente con lo que ve un usuario vidente. Build verificado OK; verificado visual/funcionalmente por el usuario (form de ausencias + alta de feriado). Con esto **la Etapa 2 queda completa** y no quedan deudas accionables en el registro (el resto son casos 💤 aceptados).
...
- **#41** — Wrapper "label + `InputTiempo`" duplicado — **✅** 2026-06-03, branch `fix/deudas-frontend`. Opción "prop `label` en `InputTiempo`" (no primitivo nuevo): es §7.4 (variante sobre primitivo existente) y reduce código; se actuó al 2º uso (práctica del proyecto, cf. #14). `InputTiempo` suma `label` opcional → con label se auto-envuelve en `<label>`+eyebrow (espejo de `Field`, asociación implícita, sin `aria-label`); sin label queda pelado idéntico a hoy (4 usos pelados intactos). Borrado el `CampoTiempo` local de `TabBarberos` (callsites → `InputTiempo label` en `<div flex:1 minWidth:180>`); inline de `BloqueFeriados` → `<InputTiempo label="Fecha">`. Eyebrow copiado 3 → 2 veces. **Cierra la Etapa 2** y deja el registro sin deudas accionables (resto 💤).
