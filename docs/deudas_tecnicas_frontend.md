...
**Follow-up de frontend (#32-FE — chat de frontend dedicado, NO Etapa 7 backend):** modal de "Completar" en `SeccionTurnero` que pida `forma_pago` (+ precio editable con default = precio del servicio, + propina) y llame al endpoint nuevo vía `apiFetch` (token admin); fila `forma_pago` en el modal de detalle; promover `BadgeFormaPago` a primitivo (5to uso: Caja, Ventas, Gastos, Planillas, Turnero); cablear `api.js` (`completarAdminTurno`).
