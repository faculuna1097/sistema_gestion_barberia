...
**Centralización (convención CLAUDE.md):** se creó el helper `getAdminHorarioAtencion()` en `api.js` (no existía) y, de paso, se migraron los **dos GET inline** que ya consumían el endpoint (`TabBarberos` y `BloqueHorarioAtencion`) — el PUT de `BloqueHorarioAtencion` (con `confirmar_cascada`/409) queda inline por ser otra responsabilidad. Build verificado OK; verificado funcionalmente por el usuario. **Cierra la Etapa 7.**
...
### #47 — `TabServicios` no usa el helper `getServiciosAdmin` · Baja · 🔲
