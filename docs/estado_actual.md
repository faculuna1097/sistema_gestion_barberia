...
Última actualización: 2026-06-08 (migración del envío de mail a la API HTTP de Resend — Fases 1/2/4 de [`plan_entregabilidad_mail.md`](./plan_entregabilidad_mail.md)). Previa: 2026-06-07 (consolidación de las deudas vivas del frontend desde el registro archivado `deudas_tecnicas_frontend.md`, preparando el merge de `feature/turnero` → `main`).
...
- **Mailing del turnero — migrado a la API HTTP de Resend ✅ (código); activación en prod pendiente.** El `services/mail/mailer.js` (movido desde `services/mailer.js`) ya no usa Nodemailer + SMTP de Gmail: delega el envío en una capa de proveedor (`mail/mailProvider.js` contrato agnóstico → `mail/resendProvider.js`, `fetch` a `api.resend.com` con timeout de 15 s). Mantiene todo el HTML y los 5 tipos de mail; suma `From` por-tenant (display name = nombre del negocio) y `text` (multipart). Motivo del cambio: Railway Hobby bloquea el SMTP saliente → en prod no salía ningún mail. Dominio de envío propio `send.barbermanager.app` **verificado** (SPF/DKIM/MX en verde; zona DNS en Vercel). Commit `ecc5484`, validado end-to-end contra Resend con `onboarding@resend.dev`. **Pendiente para activarlo en prod:** cargar `RESEND_API_KEY` + `MAIL_FROM` en Railway (web service **y** servicio cron de recordatorios) + deploy; después la verificación de entregabilidad (mail-tester/MXToolbox/prueba real Gmail+Outlook) y el DMARC. Plan y estado vivo en [`plan_entregabilidad_mail.md`](./plan_entregabilidad_mail.md). Esto **desbloquea** la Etapa 3 del recordatorio ([`plan_recordatorio_turnos.md`](./plan_recordatorio_turnos.md)).
