...
- **`MiniCalendario` del turnero — días cerrados y feriados** ✅ resuelto. El calendario (reserva y reprogramación) grisa los días que el negocio no atiende y los feriados, usando `horario_atencion` + `feriados` de `GET /api/turnero/tenant` (antes el domingo estaba hardcodeado). Pendiente sólo si se quiere precisión por barbero: grisar días donde el negocio abre pero el barbero elegido no tiene slots — requiere un endpoint de disponibilidad por día (ver `sistema_de_disenio.md` §9 #3).
