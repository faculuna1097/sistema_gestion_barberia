...
- **`/api/admin/turnos` y `/api/admin/planilla` aceptan token operativo.** Ninguna de las dos rutas tiene `requiereRol` (en `index.js` van solo con `verificarToken`), así que cualquier JWT válido pasa — incluido el operativo del iPad, que vive permanentemente en `localStorage`. El scoping por rol dentro del controller solo distingue `barbero` (filtra por `req.barbero_id`); cualquier otro rol cae en la rama que lee `barbero_id` del query param, de modo que un token operativo podría leer la planilla económica o los turnos de cualquier barbero pasándole su id. Es **pre-existente** (no lo introduce el acceso de barberos al panel) y de bajo riesgo real (el token operativo ya está detrás del login del iPad), pero la hardening limpia es ajustar ambas a `requiereRol('admin','barbero')` para excluir operativo. Antes de aplicarlo hay que confirmar que ningún flujo del iPad llame a esas rutas con token operativo — en principio sí es seguro: el flujo operativo usa `/api/turnos` y `/api/cortes`, no `/api/admin/*`. Detectada en la Fase 3 del acceso de barberos al panel; **planificada para resolverse en la Fase 6** (que ya toca esas secciones y audita sus `requiereRol`).
