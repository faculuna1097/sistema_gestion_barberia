...
Última actualización: 2026-05-13 (Paso 5 del turnero — endpoints públicos `/api/turnero/*`).
...
- **`POST /api/turnero/turnos` y `/reprogramar` no validan que `inicio` caiga en un slot real.** Confían en `/api/turnero/disponibilidad` + el frontend del turnero para elegir solo slots válidos. El constraint EXCLUDE GIST protege contra solapamientos con otros turnos, pero un `inicio` fuera de bloque horario o sobre una suspensión se aceptaría si no choca con otro turno reservado. Blindar requeriría reejecutar el algoritmo en cada POST. Postergado.
