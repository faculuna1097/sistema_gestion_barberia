...
Última actualización: 2026-05-21 (Fase 2 del plan de horario de atención — backend de feriados — completada en branch `feature/horario-atencion`).
...
- `loginAdmin()` en `api.js` distingue 402 (`err.bloqueado = true`) del resto de errores.
...
- **El `MiniCalendario` del turnero tiene el día cerrado hardcodeado (domingo).** El calendario de selección de fecha en `frontend-turnero` grisa el domingo de forma fija, sin consultar `tenant.horario_atencion` ni `tenant.feriados`. Resultado observado tras la Fase 2: un día declarado feriado NO sale en gris — sigue clickeable y sólo aparece vacío de horarios (el backend ya devuelve `[]` correctamente, pero la UI no lo anticipa). Corregir cuando se retome la "Deuda 1+2 del rediseño del turnero" (postergada, ver `plan_horario_atencion.md` §5): el calendario debe grisar tanto los días cerrados del horario semanal como los feriados, usando los datos que `GET /api/turnero/tenant` ya expone. Bajo riesgo funcional (el cliente no puede reservar igual, el backend bloquea), pero es confuso visualmente.
