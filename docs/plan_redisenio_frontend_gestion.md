...
26. **`ChipBarbero` candidato a primitivo (chip de filtro)**: implementado local en `SeccionPlanillas.jsx` (radius pill, ghost inactivo + tinted accent activo, `aria-pressed`). Es el primer caso del patrón "chip de filtro" en el admin — el resto del producto usa `TogglePill` (binario) o `Tabs` (navegación). Probable que aparezca en `SeccionGestion` (filtros por estado/categoría) o en `SeccionBalances` (filtro por barbero/período). Promover a `components/ui/` cuando aparezca el segundo uso (§7.1). Mantener sincronizado si se modifica.
