...
30. **Primitivo `Toast` pendiente** — `SeccionTurnero` necesitó banner inline para errores de acciones (cambio de estado, cancelación) porque `alert()` es bloqueante y feo. Se hizo sub-componente local `BannerError` con auto-dismiss 6s. Es el primer caso del patrón "feedback efímero de acción" en el admin. Construir primitivo `Toast` cuando aparezca el segundo uso (probable en `SeccionGestion` — feedback de guardado / borrado de productos, servicios, barberos). El primitivo debería soportar variantes `success | danger | warning | info` y posicionamiento sticky bottom-right o top-center (definir cuando se construya).
