...
17. **`EmptyState` no acepta `tone`**: el wrapper del glyph fija `color: theme.muted`, por lo que cuando se usa con `IconoAlerta` (estado de error en `SeccionInicio`), el ícono se ve gris en lugar de rojo. **Plan**: agregar prop `tone` (`'muted' | 'success' | 'danger' | 'warning'`) al primitivo `EmptyState` que pinte el color del wrapper del glyph. Backport a turnero (queda como deuda análoga a §3-13: divergencia entre fronts). Atacar cuando una pantalla lo pida fuerte o como parte de un pase de QA de la fase.
18. **Tabla ad-hoc con `role="table"/"row"/"cell"` en `SeccionInicio`**: parche de accesibilidad mientras `DataTable` no exista. Se reemplaza naturalmente cuando se construya el primitivo en Fase 5 (esperado durante Caja/Ventas/Balances).
...
*Última actualización: 2026-05-26 — `SeccionInicio` cerrada. Decisiones nuevas: D6 (admin usa `LoadingState` spinner en vez de Skeleton, primitivo nuevo en `components/ui/`) y D7 (fondo del área de contenido del admin pasa a `theme.surfaceAlt` aplicado en `PanelAdmin.jsx`, regla general — las secciones no override). Deudas nuevas: #16 `BadgeVariacion` candidato a primitivo, #17 `EmptyState` sin prop `tone`, #18 tabla ad-hoc con roles ARIA en stock (resuelve cuando exista `DataTable`). Deuda #15 marcada como resuelta vía D6. Próximo chat: `SeccionCaja`.*
